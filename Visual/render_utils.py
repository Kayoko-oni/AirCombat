...
def create_drone_model_mesh(position=None, color=(1.0, 0.0, 0.0)):
    """创建中心在原点的无人机模型（位置参数仅用于兼容，实际不平移）"""
    import open3d as o3d
    import numpy as np
    ...
    body.translate([-1.25, -1.25, arm_height])
    ...
    for motor in motors:
        combined += motor
    R = combined.get_rotation_matrix_from_xyz((np.pi, 0, 0))
